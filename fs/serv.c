...
serve_read(envid_t envid, union Fsipc *ipc) {
    ...
    int res = openfile_lookup(envid, req->req_fileid, &open_file);
    if (res < 0) { return res; }

    ssize_t read = file_read(open_file->o_file, ipc->readRet.ret_buf, req->req_n, open_file->o_fd->fd_offset);
    if (read < 0) { return read; }

    open_file->o_fd->fd_offset += read;
    return read;
}
...
serve_write(envid_t envid, union Fsipc *ipc) {
    ...
    ssize_t writen = file_write(open_file->o_file, req->req_buf, req->req_n, open_file->o_fd->fd_offset);
    if (writen < 0) { return writen; }

    open_file->o_fd->fd_offset += writen;
    return writen;
}
